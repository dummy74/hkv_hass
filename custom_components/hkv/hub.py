...
class HKVHub:
    ...
    async def fetch_data(self, hass):
        ...
        try:
            ...
            tasks = [
                self.hkv.get_status(dst=0, timeout=10),
            ]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            state_pck = results[0][1] if isinstance(results[0], tuple) and results[0][0] else None
            if state_pck:
            ...
            tasks = [
                self.hkv.get_temps(dst=0, timeout=10),
            ]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            temps_pck = results[0][1] if isinstance(results[0], tuple) and results[0][0] else None
            if temps_pck:
            ...
            tasks = [
                self.hkv.get_relais(dst=0, timeout=10),
            ]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            relais_pck = results[0][1] if isinstance(results[0], tuple) and results[0][0] else None
            if relais_pck:
            ...
            tasks = [
                self.hkv.get_connections(dst=0, timeout=10)
            ]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            conn_pck = results[0][1] if isinstance(results[0], tuple) and results[0][0] else None
            # Handle connections
            if conn_pck:
                query_tasks = []
    ...
    async def _query_device(self, addr, dev):
        ...
        tasks = [
            self.hkv.get_status(dst=addr, timeout=10),
        ]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        ...
        tasks = [
            self.hkv.get_temps(dst=addr, timeout=10),
        ]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        ...
        tasks = [
            self.hkv.get_relais(dst=addr, timeout=10)
        ]
        results = await asyncio.gather(*tasks, return_exceptions=True)
